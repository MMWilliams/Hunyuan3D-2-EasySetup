> ## ⚡ EasySetup fork — one command to install & run
>
> This is a community fork of [Tencent/Hunyuan3D-2](https://github.com/Tencent/Hunyuan3D-2) that adds a **single setup command** which creates the environment, installs every dependency (including the ones the upstream README leaves out), compiles the texturing CUDA extensions, and launches the web UI with **both image→3D and text→3D enabled**.
>
> **Prerequisites:** [Miniconda](https://docs.conda.io/en/latest/miniconda.html), an NVIDIA GPU (8 GB+ VRAM recommended), and — for textured output — [Visual Studio Build Tools](https://visualstudio.microsoft.com/downloads/) with the "Desktop development with C++" workload (Windows) or `build-essential` + CUDA toolkit (Linux).
>
> **Windows (PowerShell):**
> ```powershell
> git clone https://github.com/MMWilliams/Hunyuan3D-2-EasySetup.git; cd Hunyuan3D-2-EasySetup; powershell -ExecutionPolicy Bypass -File .\setup.ps1
> ```
>
> **Linux / macOS:**
> ```bash
> git clone https://github.com/MMWilliams/Hunyuan3D-2-EasySetup.git && cd Hunyuan3D-2-EasySetup && bash setup.sh
> ```
>
> That's it. The script handles everything and opens the UI at **http://localhost:8080**. The first launch downloads ~10–15 GB of model weights from Hugging Face (cached afterwards, so later launches are fast).
>
> **Useful options** (`setup.ps1 -Flag` / `VAR=value bash setup.sh`):
> | What | PowerShell | bash |
> |---|---|---|
> | Install only, don't launch | `-NoLaunch` | `NO_LAUNCH=1` |
> | Different port | `-Port 7860` | `PORT=7860` |
> | Pick CUDA wheel | `-Cuda cu121` | `CUDA=cu121` |
> | Image→3D only (smaller download) | `-NoText23d` | `NO_T23D=1` |
>
> **Re-launch later** without re-running setup:
> ```
